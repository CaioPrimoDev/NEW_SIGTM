...
public class UsuarioComumService implements UsuarioComumIService {
    ...
    public boolean save(UsuarioComum usuario) {
        ...
        try {
            ...
            throw new RegraNegocioException("Já existe um usuário comum com esse CPF ou dados inválidos.");

        } catch (RuntimeException e) {
            log.error("Erro inesperado ao salvar UsuarioComum.", e);
            throw new RegraNegocioException("Erro ao salvar Usuário Comum.");
        }
    }
    ...
    public void delete(Long id) {
        if (id == null || id <= 0) {
            ...
            throw new RegraNegocioException("ID de Usuário Comum inválido.");
        }
        ...
        try {
            ...
            throw new RegraNegocioException("Usuário Comum não encontrado para exclusão.");

        } catch (RuntimeException e) {
            log.error("Erro inesperado ao excluir UsuarioComum.", e);
            throw new RegraNegocioException("Erro ao excluir Usuário Comum.");
        }
    }
    ...
    public List<UsuarioComum> findAll() {
        try {
            ...
            throw new RegraNegocioException("Erro ao buscar todos os Usuários Comuns.");
        }
    }
    private void validarUsuarioComum(UsuarioComum usuario) {
        if (usuario == null) {
            log.warn("UsuarioComum recebido é nulo.");
            throw new RegraNegocioException("Usuário Comum não pode ser nulo.");
        }
        ...
        if (StringUtil.isNullOrEmpty(usuario.getNome())) {
            ...
            throw new RegraNegocioException("O nome do Usuário Comum é obrigatório.");
        }

        if (!StringUtil.hasValidLength(usuario.getNome(), 3, 30)) {
            log.warn("Nome do UsuarioComum fora do tamanho permitido: '{}'", usuario.getNome());
            throw new RegraNegocioException("O nome deve ter entre 3 e 30 caracteres.");
        }
        ...
        try {
            return repo.findById(id)
                    .orElseThrow(() -> {
                        ...
                        return new RegraNegocioException("Usuário Comum não encontrado.");
                    });

        } catch (RuntimeException e) {
            log.error("Erro inesperado ao buscar UsuarioComum por ID.", e);
            throw new RegraNegocioException("Erro ao buscar Usuário Comum por ID.");
        }
    }
