...
public class TipoPromocaoController {
    ...
    public ResponseEntity<TipoPromocaoResponseDTO> salvar(
    ...
    public ResponseEntity<TipoPromocaoResponseDTO> atualizar(
    ...
    public ResponseEntity<TipoPromocaoResponseDTO> buscarPorId(@PathVariable Long id) {
        return ResponseEntity.ok(
                mapToResponse(service.findById(id))
        );
    }

    private TipoPromocaoResponseDTO mapToResponse(TipoPromocao entity) {
        TipoPromocaoResponseDTO dto =
                mapper.map(entity, TipoPromocaoResponseDTO.class);

        if (entity.getUsuarioCadastro() != null &&
            entity.getUsuarioCadastro().getPessoa() != null) {
            dto.setNomeUsuarioCadastro(
                    entity.getUsuarioCadastro().getPessoa().getNome()
            );
        }
        return dto;
    }
